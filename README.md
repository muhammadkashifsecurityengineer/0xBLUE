# 0xBLUE: SOC Analyst Arsenal

Author: [Muhammad Kashif | GitHub](https://github.com/muhammadkashifsecurityengineer)
...
Most "awesome list" repositories for security are plain Markdown, which means copying and pasting links one at a time. This repository ships the collection as a standard Netscape bookmark file (`bookmarks.html`), so it imports directly into Chrome, Firefox, Edge, or Brave as a ready-made bookmark folder structure, in addition to being browsable here on GitHub.

## Import instructions

**Chrome / Edge / Brave**
1. Open Bookmark Manager (`Ctrl+Shift+O` / `Cmd+Shift+O`).
2. Click the three-dot menu, select **Import Bookmarks**.
3. Choose `bookmarks.html` from this repository.

**Firefox**
1. Open the Library (`Ctrl+Shift+O` / `Cmd+Shift+O`).
2. **Import and Backup > Import Bookmarks from HTML**.
3. Select `bookmarks.html`.

## Categories

Listed in the same alphabetical order used inside `bookmarks.html`.

| Category | Focus |
|---|---|
| Attack Reference Material | Active Directory and Kerberos attack techniques (Kerberoasting, DCSync, Golden/Silver Ticket, ADCS abuse, Zerologon, Golden SAML) |
| Blogs | Ongoing threat research and DFIR write-ups |
| Books | Long-form published material on SOC operations and network security monitoring |
| Event Log References | Vendor-specific log field and event ID documentation (Windows, Sysmon, cloud platforms, firewalls, proxies, WAFs, identity providers, and SaaS platforms like Microsoft 365, Google Workspace, Slack, GitHub, and Salesforce) |
| Good Reads | Short-form reading on SOC analyst practice and alert fatigue |
| Incident Response Frameworks and Detection Concepts | NIST and CISA IR guidance, Pyramid of Pain, MITRE ATT&CK Navigator, CAR, Engage |
...
| Resources and Reference Material | Frameworks, standards, and living-off-the-land references (MITRE ATT&CK, D3FEND, CAPEC, Sigma, Atomic Red Team, OWASP, NIST CSF, CIS Benchmarks, Microsoft Security Compliance Toolkit) |
...
- Every link is verified reachable and current as of the last update. Tools that have been shut down or superseded (for example, the original `dnSpy` project or `CRXcavator`) are replaced with their actively maintained successors rather than left as dead links.
...
- Categories are sorted alphabetically, and links within each category are sorted alphabetically, so new pull requests should slot into the correct position rather than being appended to the end.
...
This is a link collection, not original software or written content. Feel free to fork, reorganize, or build on it.
